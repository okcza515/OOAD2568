...
func (handler AddStudentStrategy) Execute() error {
	...
	startDate := validator.Field(validation.FieldConfig{Name: "startDate", Prompt: "Enter student start date (YYYY-MM-DD): "}).Required().IsDate().GetInput()
	birthDate := validator.Field(validation.FieldConfig{Name: "birthDate", Prompt: "Enter student birth date (YYYY-MM-DD): "}).Required().IsDate().GetInput()
	program := validator.Field(validation.FieldConfig{Name: "program", Prompt: "Enter student program (e.g., Regular, International): "}).Required().GetInput()
