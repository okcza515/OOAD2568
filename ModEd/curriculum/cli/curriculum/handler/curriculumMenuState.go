...
func NewCurriculumMenuState(handler *curriculumHandler, parent MenuState) *CurriculumMenuState {
	...
	state.AddMenuItem("1", "Create Seed Curriculum", func() (MenuState, error) {
		err := handler.createSeedCurriculum()
		return state, err
	})

	state.AddMenuItem("2", "List all Curriculums", func() (MenuState, error) {
		err := handler.listCurriculums()
		return state, err
	})

	state.AddMenuItem("3", "Get Curriculum by Id", func() (MenuState, error) {
		err := handler.getCurriculumById()
		return state, err
	})

	state.AddMenuItem("4", "Update Curriculum by Id", func() (MenuState, error) {
		err := handler.updateCurriculumById()
		return state, err
	})

	state.AddMenuItem("5", "Delete Curriculum by Id", func() (MenuState, error) {
