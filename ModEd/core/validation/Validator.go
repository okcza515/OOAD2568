...
const (
	...
	dateTimeRegexString       = `^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$`
	kmuttStudentIDRegexString = `^\d{11}$`
)
...
func (v *Validator) IsDateTimeValid(dateTime string) bool {
	...
	dateTimeRegex := lazyRegexCompile(dateTimeRegexString)()
	if dateTimeRegex == nil {
		return false
	}
...
func (v *Validator) IsStudentID(studentId string) bool {
	...
	dateTimeRegex := lazyRegexCompile(kmuttStudentIDRegexString)()
	if dateTimeRegex == nil {
		return false
	}
	return dateTimeRegex.MatchString(studentId)
}
