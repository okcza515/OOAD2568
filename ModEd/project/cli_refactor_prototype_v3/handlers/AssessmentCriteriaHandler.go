...
func (handler *AssessmentCriteriaHandler) ImportCSV(io *core.MenuIO) {
	...
	for i, record := range records {
		...
		if criteriaName := strings.TrimSpace(record[0]); criteriaName != "" {
			if err := handler.instanceStorer.AssessmentCriteria.InsertAssessmentCriteria(criteriaName); err != nil {
				io.Println(fmt.Sprintf("Row %d: Error adding criteria '%s': %v", i+1, criteriaName, err))
			} else {
				successCount++
			}
		}
	...
	io.Println(fmt.Sprintf("Import complete. Added %d criteria.", successCount))
}
